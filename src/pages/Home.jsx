    ...
    <>
      <section className="mx-auto grid min-h-screen max-w-[1180px] items-center gap-12 px-5 pb-20 pt-32 lg:grid-cols-[1.12fr_.88fr]">
        <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7 }}>
          ...
          <p className="eyebrow">Full-stack · ML · Developer</p>
          <h1 className="hero-title">Muhammad<span>Saim.</span></h1>
          <p className="mt-8 max-w-2xl text-lg leading-8 text-muted">Building <strong className="font-semibold text-primary">intelligent systems</strong> where full-stack engineering, machine learning, and thoughtful product design meet.</p>
        ...
        <motion.div initial={{ opacity: 0, scale: .94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .8, delay: .15 }} className="relative"><div className="corner-frame"><HeroScene /></div><div className="float-badge -left-4 top-16">NEXT.JS · PYTHON</div><div className="float-badge -right-3 top-8 text-green">▲ 0 → 1 BUILDER</div><div className="float-badge -right-5 bottom-20">⚙ THREE.JS · AI</div><div className="mt-5 flex justify-center gap-2"><a className="social-button" href="mailto:" aria-label="Email"><Mail size={15} /></a><a className="social-button" href="#" aria-label="GitHub"><Github size={15} /></a><a className="social-button" href="#" aria-label="LinkedIn"><Linkedin size={15} /></a></div></motion.div>
      </section>
