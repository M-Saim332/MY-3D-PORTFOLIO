...
export default function ContactSection() {
  ...
  const submit=(event)=>{event.preventDefault();const form=new FormData(event.currentTarget);setSent(true);window.location.href=`mailto:?subject=${encodeURIComponent(`Portfolio enquiry from ${form.get('name')}`)}&body=${encodeURIComponent(`${form.get('message')}\n\nReply to: ${form.get('email')}`)}`}
