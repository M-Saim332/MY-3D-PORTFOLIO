  ...
  return (
    <motion.article initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .15 }} transition={{ delay: index * .08 }} className="project-card group">
      <div className="project-cover" style={{ '--project-color': project.color }}>
        {project.image ? <img className="h-full w-full object-cover transition duration-500 group-hover:scale-105" src={project.image} alt={`${project.title} preview`}/> : <div className="mock-window"><div className="mock-sidebar" /><div className="mock-content"><span /><span /><span /><span /></div></div>}
        <div className="absolute inset-x-5 top-5 flex justify-between font-mono text-[10px] tracking-widest text-muted"><span>{project.id}</span>{project.liveUrl || project.sourceUrl ? <a href={project.liveUrl || project.sourceUrl} target="_blank" rel="noreferrer" aria-label={`Open ${project.title}`}><ArrowUpRight size={14} /></a> : <ArrowUpRight size={14} />}</div>
      </div>
